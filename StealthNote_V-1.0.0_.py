...
LOG_FILE = os.path.join(DATA_DIR, "SteathNote_log.txt")
...
class FileManager:
    ...
    def open_ectf(path, password):
        try:
            ...
            decrypted_text = EncryptionHelper.decrypt_data(password, encrypted_data)
...
class Themes:
   ...
   "Retro Pixel": {
    ...
    @staticmethod
    def get_theme(name):
        return Themes.available_themes.get(name, Themes.available_themes["Dark"])
...
class StealthNoteApp:
    def __init__(self, top):
        ...
        self.current_font_size = self.settings.get("font_size", 11)
    ...
    def setup_ui(self):
        self.text = tk.Text(self.top, wrap=self.wrap_mode, font=("Consolas", self.current_font_size))
    ...
    def setup_menu(self):
        ...
        settings_menu.add_command(label="Toggle Word Wrap", command=self.toggle_wrap)
    ...
    def show_about(self):
        changelog = (
            ...
            "\nRecent Updates:\n"
            "- Themes\n"
            "- Toggle Word Wrap added\n"
            "- Set default open/save directory\n"
    ...
    def ask_password(self, prompt="Enter password"):
        while True:
            pw = simpledialog.askstring("🔐 Password Required", prompt, show='*')
            if not pw:
                raise ValueError("Password input canceled or empty.")
            
            # Password validation
            if self.is_strong_password(pw):
                return pw.strip()
            else:
                messagebox.showerror("Error", "Password must be at least 12 characters long, contain uppercase and lowercase letters, a number, and a special character.")
    ...
    def open_ectf(self):
        ...
        try:
            ...
            # Check if the error is related to decryption failure (incorrect password)
            if "Failed to open or decrypt file" in str(e):
                Logger.log_action("Failed to open file: Incorrect password")
    ...
    def change_font_size(self):
        new_size = simpledialog.askinteger("Change Font Size", "Enter new font size:", minvalue=5, maxvalue=50, initialvalue=self.current_font_size)
        if new_size:
    ...
    def reset_to_defaults(self):
        # Define the default settings directly in the code
        default_settings = {
            "mode": "dark",
            "font_size": 11,
            "wrap": True,
        ...
        # Apply theme and other UI changes based on the new settings
        self.apply_theme()
        self.text.configure(wrap=self.wrap_mode)
        self.text.configure(font=("Consolas", self.current_font_size))

        # Save the default settings to the settings file
        SettingsManager.save_settings(self.settings)   # Use the actual save_settings function here
